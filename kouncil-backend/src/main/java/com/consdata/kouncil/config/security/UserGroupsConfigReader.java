...
public final class UserGroupsConfigReader {
    ...
    @Value("${kouncil.authorization.role-admin:#{null}}")
    private String adminRoles;
    @Value("${kouncil.authorization.role-editor:#{null}}")
    private String editorRoles;
    @Value("${kouncil.authorization.role-viewer:#{null}}")
    ...
    public void init() {
        ...
        Optional.ofNullable(adminRoles)
                .ifPresent(roles -> roleMapping.put(KouncilRole.ROLE_KOUNCIL_ADMIN, new HashSet<>(Arrays.asList(adminRoles.split(VALUES_SEPARATOR)))));
        Optional.ofNullable(editorRoles)
                .ifPresent(roles -> roleMapping.put(KouncilRole.ROLE_KOUNCIL_EDITOR, new HashSet<>(Arrays.asList(editorRoles.split(VALUES_SEPARATOR)))));
        Optional.ofNullable(viewerRoles)
                .ifPresent(roles -> roleMapping.put(KouncilRole.ROLE_KOUNCIL_VIEWER, new HashSet<>(Arrays.asList(viewerRoles.split(VALUES_SEPARATOR)))));
        ...
        if (!roleMapping.isEmpty()) {
            List<SystemFunctionName> adminFunctions = List.of(SystemFunctionName.BROKERS_LIST, SystemFunctionName.BROKER_DETAILS,
                    ...
                    SystemFunctionName.USER_GROUPS, SystemFunctionName.USER_GROUPS_LIST, SystemFunctionName.USER_GROUP_CREATE,
                    SystemFunctionName.USER_GROUP_UPDATE,
